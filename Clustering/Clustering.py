...
if __name__=='__main__':
    centers=[[1,1,1],[3,4,3],[7,6,5],[1,5,7]] # 用于产生聚类的中心点, 聚类中心的维度代表产生样本的维度
    ...
    for i in range(row):
        ...
        if d<len(hc1.clusters[3]) and hc1.clusters[3][d]==i:
            labels_pred1.append(3)
            d=d+1


   

    hc2 = AverageLinkage(X, 4)
    print(hc2.clusters)
    ...
    for i in range(row):
        if a<len(hc2.clusters[0]) and hc2.clusters[0][a]==i:
            labels_pred2.append(0)
            a=a+1

        if b<len(hc2.clusters[1]) and hc2.clusters[1][b]==i:
            labels_pred2.append(1)
            b=b+1

        if c<len(hc2.clusters[2]) and hc2.clusters[2][c]==i:
            labels_pred2.append(2)
            c=c+1

        if d<len(hc2.clusters[3]) and hc2.clusters[3][d]==i:
            labels_pred2.append(3)
            d=d+1


    plot_data(X,labels_true)
    plot_data(X,labels_pred)
    plot_data(X,labels_pred1)
    plot_data(X,labels_pred2)
